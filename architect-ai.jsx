...
export default function ArchitectAI() {
  ...
  useEffect(() => {
    ...
    api('/api/ecosystem').then(data => {
      ...
      if (data.latestBuildId)    setLatestBuildId(data.latestBuildId);
  ...
  const implAbortRef  = useRef(null); // AbortController for the active SSE stream
  const [latestBuildId, setLatestBuildId] = useState(null);
  // Live build poll status — null while no poll in flight
  // { status: 'queued'|'building'|'complete'|'failed', pipelineUrl?, images?, error? }
  const [buildPollStatus, setBuildPollStatus] = useState(null);
  ...
  // Auto-start build polling when the app loads with a pending build
  // Must be declared before early returns to satisfy Rules of Hooks
  useEffect(() => {
    if (!hydrated || !latestBuildId || implRunning) return;
    startBuildPolling(latestBuildId, { immediate: true });
    return () => { if (buildPollRef.current) clearInterval(buildPollRef.current); };
  }, [hydrated, latestBuildId]);
  ...
  async function implement() {
    ...
    // Persist repoName to the server scratch-pad before the agent reads it
  ...
  async function runImplStream() {
    ...
    try {
      const token   = await getAccessTokenSilently();
      const baseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:3001';
      const response = await fetch(`${baseUrl}/api/implement/start`, {
        method:  'POST',
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (event.type === 'queued') { buildId = event.buildId; succeeded = true; }
            if (event.type === 'error')  { succeeded = false; }
          } catch {}
        }
      }
    } catch (err) {
      if (err.name !== 'AbortError') {
        setImplLog(prev => [...prev, { type: 'error', message: err.message }]);
      }
    }

    implAbortRef.current = null;

    if (succeeded && buildId) {
      // SSE closed after queuing — keep implRunning=true until build reaches terminal status
      setLatestBuildId(buildId);
      setBuildPollStatus({ status: 'queued' });
      setImplLog(prev => [...prev, { type: 'queued', buildId, message: 'Build queued — github-provider is processing' }]);
      setSvcStatuses(prev => {
        const next = { ...prev };
        services.forEach(s => { next[s.id] = 'building'; });
        return next;
      });
      startBuildPolling(buildId, { immediate: true });
    } else {
      // SSE ended in error or cancel — release the button
  ...
  function killImpl() {
    const wasStreaming = !!implAbortRef.current;
    if (implAbortRef.current) {
      implAbortRef.current.abort();
      implAbortRef.current = null;
    }
    ...
    setBuildPollStatus(null);
    setImplRunning(false);
    setImplLog(prev => [...prev, { type: 'info', message: wasStreaming ? 'Cancelled' : 'Stopped watching build' }]);
  }

  function startBuildPolling(buildId, { immediate = false } = {}) {
    if (buildPollRef.current) clearInterval(buildPollRef.current);

    const poll = async () => {
      try {
        const data = await api(`/api/build/${encodeURIComponent(buildId)}`);
        ...
        if (data.status === 'complete') {
          ...
          setSvcStatuses(prev => {
            const next = { ...prev };
            services.forEach(s => { next[s.id] = 'built'; });
            return next;
          });
          setImplLog(prev => [...prev, {
            type: 'build_complete',
          ...
          setSvcStatuses(prev => {
            const next = { ...prev };
            services.forEach(s => { next[s.id] = 'build_failed'; });
            return next;
          });
          setImplLog(prev => [...prev, {
            type: 'build_failed',
          ...
          setMessages(prev => [...prev, { role: 'assistant', content:
            `Build failed${data.error ? ': ' + data.error : ''}.\n\nDescribe what you'd like to fix or paste the error logs and I'll diagnose the issue.`,
          }]);
          setView('chat');

        } else {
          // Still in-flight — update live status (queued or building)
          setBuildPollStatus({
            status:      data.status === 'building' ? 'building' : 'queued',
            pipelineUrl: data.pipelineUrl || null,
          });
        }
      } catch {}
    };
    ...
    if (immediate) poll();
    buildPollRef.current = setInterval(poll, 5000);
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: C.bg, color: C.text, fontFamily: "'IBM Plex Sans', sans-serif", overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          {!!services.length && (
            <button onClick={implement} disabled={implRunning}
              ...
              {buildPollStatus ? '⟳ ci…' : implRunning ? '⟳ implementing…' : '▶ implement'}
            </button>
          )}
          ...
          {latestBuildId && !implRunning && (
            <button onClick={() => startBuildPolling(latestBuildId, { immediate: true })}
      ...
      {showImplPanel && (
        <div style={{ background: C.bg, borderBottom: `1px solid ${C.border}`, flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              {implRunning && (
                <button onClick={killImpl}
                  ...
                  {buildPollStatus ? '✕ STOP WATCHING' : '✕ KILL'}
                </button>
              )}
          ...
          <div style={{ maxHeight: '280px', overflow: 'auto', padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: '3px' }}>
            {implLog.map((entry, i) => {
              ...
              if (entry.type === 'push' || entry.type === 'progress') return (
              ...
              if (entry.type === 'queued') return (
                <div key={i} style={{ color: C.accentBr, fontFamily: 'IBM Plex Mono', fontSize: '10px', paddingTop: '6px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <span style={{ display: 'inline-flex', gap: '2px' }}>
                    <span className="dot" /><span className="dot" /><span className="dot" />
                  </span>
                  {entry.message}
              ...
              if (entry.type === 'build_complete') return (
                <div key={i} style={{ paddingTop: '6px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  <div style={{ color: C.green, fontFamily: 'IBM Plex Mono', fontSize: '11px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ✓ {entry.message}
                    {entry.pipelineUrl && (
                      <a href={entry.pipelineUrl} target="_blank" rel="noreferrer"
                        style={{ color: C.accentBr, textDecoration: 'underline', fontFamily: 'IBM Plex Mono', fontSize: '10px', fontWeight: '400' }}>
                        view pipeline ↗
                      </a>
                    )}
                  </div>
                  {entry.images?.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', paddingLeft: '8px', paddingTop: '2px' }}>
                      <div style={{ color: C.dim, fontFamily: 'IBM Plex Mono', fontSize: '9px', fontWeight: '700', letterSpacing: '0.14em', marginBottom: '2px' }}>IMAGES</div>
                      {entry.images.map((img, j) => (
                        <div key={j} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <code style={{ color: C.accentGlow, fontFamily: 'IBM Plex Mono', fontSize: '10px', background: C.surface, border: `1px solid ${C.border}`, borderRadius: '3px', padding: '1px 6px', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {img.image}
                          </code>
                          <button
                            onClick={() => navigator.clipboard.writeText(img.image)}
                            title="Copy image name"
                            style={{ background: 'none', border: `1px solid ${C.border}`, borderRadius: '3px', color: C.muted, cursor: 'pointer', fontFamily: 'IBM Plex Mono', fontSize: '10px', padding: '1px 6px', flexShrink: 0 }}>
                            ⎘
                          </button>
                        </div>
                      ))}
              ...
              if (entry.type === 'build_failed') return (
                <div key={i} style={{ paddingTop: '6px', display: 'flex', flexDirection: 'column', gap: '3px' }}>
                  <div style={{ color: C.red, fontFamily: 'IBM Plex Mono', fontSize: '11px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ✗ {entry.message}
                    {entry.pipelineUrl && (
                      <a href={entry.pipelineUrl} target="_blank" rel="noreferrer"
                        style={{ color: C.accentBr, textDecoration: 'underline', fontFamily: 'IBM Plex Mono', fontSize: '10px', fontWeight: '400' }}>
            ...
            {/* Live build poll status — shows while polling, clears on terminal */}
            {buildPollStatus && (
              <div style={{ marginTop: '4px', paddingTop: '6px', borderTop: `1px solid ${C.border}` }}>
                {buildPollStatus.status === 'queued' && (
                  <div style={{ color: C.muted, fontFamily: 'IBM Plex Mono', fontSize: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ display: 'inline-flex', gap: '2px' }}>
                      <span className="dot" /><span className="dot" /><span className="dot" />
                    </span>
                ...
                {buildPollStatus.status === 'building' && (
                  <div style={{ color: C.accentBr, fontFamily: 'IBM Plex Mono', fontSize: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ display: 'inline-flex', gap: '2px' }}>
                      <span className="dot" /><span className="dot" /><span className="dot" />
                    </span>
                    CI pipeline running
                    {buildPollStatus.pipelineUrl && (
                      <a href={buildPollStatus.pipelineUrl} target="_blank" rel="noreferrer"
                        style={{ color: C.accentBr, textDecoration: 'underline' }}>
                        view ↗
                      </a>
                    )}
                  </div>
